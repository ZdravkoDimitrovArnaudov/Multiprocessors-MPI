...
int main(int argc, char* argv[]) {
    ...
    if (yo == 0) //receiver
    {
    	start_time = MPI_Wtime(); //inicio tiempo
        sleep(6);
        MPI_Recv(array_receive, num_int, MPI_INT, 1, tag, MPI_COMM_WORLD, &status);
        sleep(1);
        MPI_Recv(array_receive, num_int, MPI_INT, 1, tag, MPI_COMM_WORLD, &status);
        end_time = MPI_Wtime(); //fin tiempo
    ...
    */
    	start_time = MPI_Wtime(); //inicio tiempo
    	sleep(1); //computo
    	MPI_Send(array_int, num_int , MPI_INT, 0, tag, MPI_COMM_WORLD); //mandamos el primer mensaje
    	sleep(6);
    	MPI_Send(array_int, num_int , MPI_INT, 0, tag, MPI_COMM_WORLD); //mandamos el primer mensaje
    	end_time = MPI_Wtime(); //fin tiempo
    	printf("La emisión ha tardado :%lf\n",end_time-start_time);
