/*
 * Ejer1_C.c
 * Hay varios emisores y en este caso se seleccionan los mensajes mediante id y tag.
 * 
 * 
 */
	...
	if (me == 0){ //receiver
		...
		for (int num_pc = 1; num_pc < p; num_pc++) {
			for (int tag = 9; 0 <= tag; tag--){
			 	MPI_Recv(&buff_number, 1, MPI_INT, num_pc, tag, MPI_COMM_WORLD, &status);
				printf ("Message %d received from %d: %d\n", tag, num_pc, buff_number);
			}
		}
